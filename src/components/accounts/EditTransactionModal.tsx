...
export function EditTransactionModal({ transaction, isOpen, onClose }: EditTransactionModalProps) {
  ...
  useEffect(() => {
    if (transaction) {
      ...
      const allKnownCats = [...INCOME_CATEGORIES, ...EXPENSE_CATEGORIES].map(c => c.value);
      if (allKnownCats.includes(transaction.category)) {
        setCategory(transaction.category);
        setOtherCategory("");
      } else {
        setCategory("other");
        setOtherCategory(transaction.category);
      }
    }
  }, [transaction, isOpen]);
      ...
      <DialogContent className="sm:max-w-[450px] rounded-[2.5rem] p-0 overflow-hidden border-none shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {mutation.isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : "Update Entry"}
            </Button>
          </div>
        </form>
      </DialogContent>
